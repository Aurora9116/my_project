...
func (p *ProjectDao) DeleteProjectCollect(ctx context.Context, memId int64, projectCode int64) error {
	return p.conn.Default(ctx).Where("member_code = ? and project_code = ?", memId, projectCode).Delete(&data.ProjectCollection{}).Error
}
...
func (p *ProjectDao) UpdateDeletedProject(ctx context.Context, projectCode int64, deleted bool) error {
	session := p.conn.Default(ctx)
	if deleted {
		return session.Model(&data.Project{}).Where("id=?", projectCode).Update("deleted", 1).Error
	}
	return session.Model(&data.Project{}).Where("id=?", projectCode).Update("deleted", 0).Error
}
